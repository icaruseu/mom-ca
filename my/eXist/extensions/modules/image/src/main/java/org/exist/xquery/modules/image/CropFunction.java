...
public class CropFunction extends BasicFunction {
    ...
    public final static FunctionSignature signature = new FunctionSignature(
        ...
        new FunctionReturnSequenceType(Type.STRING, Cardinality.ZERO_OR_ONE, "the cropd image or an empty sequence if $image is invalid")
    );
    ...
    public Sequence eval(Sequence[] args, Sequence contextSequence) throws XPathException {
		...
		return new StringValue("Error");
            ...
            //crop the image
            Image cropImage = Toolkit.getDefaultToolkit().createImage(new FilteredImageSource(image.getSource(), new CropImageFilter(x1, y1, y2, x2)));
            if(cropImage instanceof BufferedImage) {
                // just in case cropImage is allready an BufferedImage
                bImage = (BufferedImage)cropImage;
            } else {
                bImage = new BufferedImage(cropImage.getWidth(null),cropImage.getHeight(null),
