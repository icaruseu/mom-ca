...
declare function account:confirm($code as xs:string) as xs:string {
    ...
    let $create-user-account := 
        if(not(sm:user-exists($email))) then sm:create-account($email, $password, $account:user-groups)
        else sm:passwd($email, $password)
