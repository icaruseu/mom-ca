...
declare variable $xrx:servername := substring-before(substring-after(request:get-url(), '//'), '/');
(: sometimes it is helpful to have the complete URL :)
declare variable $xrx:http-request-root := concat('//', $xrx:servername, concat('/', $xrx:platform-id, '/'));
(: 
...
declare variable $xrx:localhost-request-base-url :=
    concat('//localhost:', conf:param('jetty-port'), conf:param('request-root'));
declare variable $xrx:jetty-request-base-url :=
    concat('//', conf:param('jetty-servername'), ':', conf:param('jetty-port'), conf:param('request-root'));
declare variable $xrx:http-icon-root := concat('//', request:get-server-name(), $xrx:port, concat('/', $xrx:platform-id, '/'), 'icon/');    
