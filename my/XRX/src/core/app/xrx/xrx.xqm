...
declare variable $xrx:user-id := if(request:get-header('userid') != '') then xmldb:decode(request:get-header('userid')[1]) else sm:id()//username/text();
...
declare function xrx:xformsflag() as xs:boolean {
    ...
    if($xrx:mode = 'mainwidget') then
        ...
        return
        if(($flag = 'true' and sm:id()//username/text() != 'guest') or matches($xrx:tokenized-uri[last()], '(registration|request-password|iipmooviewer)')) then true()
        else false()
