	...
	_newEditAttribute: function(name, label, value) {
		...
		var self = this,
			...
			newEditAttributeInput = $('<input></input>')
				...
				.attr("value", value)
				.attr("name", name),
			newValueSelected = $('<select></select>')
				.addClass(uiFormsTableCellClass)
				.attr("value", value)
				.attr("name", name),
			newValueOption = $('<option>' + suggestedValue[0].value +'</option>')
				.attr("title", value),
			newEditAttributeTrash = $('<div><span class="ui-icon ui-icon-trash"/></div>').
				addClass(uiFormsTableCellClass);
		if (suggestedValue){
			
			newValueSelected.append(newValueOption);
			
			newEditAttribute.append(newEditAttributeLabel)
			.append(newValueSelected)
			.append(newEditAttributeTrash);
			console.log('HIIIIIIIIIIIIII');
			console.log(suggestedValue[0].value);
		} else {
