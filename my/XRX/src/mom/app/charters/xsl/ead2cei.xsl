...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ead="urn:isbn:1-931666-22-9" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:cei="http://www.monasterium.net/NS/cei" xmlns:fn="http://www.w3.org/2005/xpath-functions" version="1.0" id="ead2cei">
    <!-- Script to convert ead (charters) to cei -->
    <xsl:template match="/">
        <xsl:apply-templates/>
    </xsl:template>
    ...
    <xsl:template match="ead:c[@level='item']">
        <cei:text>
            ...
            <xsl:attribute name="id">
                <xsl:variable name="i" select="translate(./ead:did/ead:unitid/text(), '{', '')"/>
    ...
    <xsl:template match="ead:c[@level='item']/ead:did">
        ...
        <cei:body>
            ...
            <xsl:if test="./ead:unitid[@encodinganalog='cei:altIdentifier']/text()">
            ...
            <cei:chDesc>
                ...
                <cei:witnessOrig>
                    ...
                    <cei:auth>
                        <xsl:if test="./ead:physdesc/ead:physfacet[@encodinganalog='cei:seal']">
                            <xsl:variable name="k" select="./ead:physdesc/ead:physfacet[@encodinganalog='cei:seal']"/>
                            <xsl:for-each select="$k">
                                <xsl:call-template name="seal"/>
                            </xsl:for-each>
                        </xsl:if>
                        <xsl:if test="./ead:physdesc/ead:physfacet[@encodinganalog='cei:sealDesc']">
                            <xsl:variable name="i" select="./ead:physdesc/ead:physfacet[@encodinganalog='cei:sealDesc']"/>
                            <xsl:for-each select="$i">
                                <xsl:call-template name="seal"/>
                            </xsl:for-each>
                        </xsl:if>
                        <xsl:if test="../ead:odd[@encodinganalog='cei:notarialSub']">
                            <xsl:variable name="j" select="../ead:odd[@encodinganalog='cei:notarialSub']"/>
                            <xsl:for-each select="$j">
                                <xsl:call-template name="noterial"/>
                            </xsl:for-each>
                        </xsl:if>
                    </cei:auth>
                    <cei:physicalDesc>
                        <xsl:if test="./ead:physdesc/ead:physfacet[@encodinganalog='cei:material']">
                            <xsl:variable name="o" select="./ead:physdesc/ead:physfacet[@encodinganalog='cei:material']"/>
                            <xsl:for-each select="$o">
                                <xsl:call-template name="material"/>
                            </xsl:for-each>
                        </xsl:if>
                        <xsl:if test="./ead:physdesc/ead:physfacet[@encodinganalog='cei:condition']">
                            <xsl:variable name="t" select="./ead:physdesc/ead:physfacet[@encodinganalog='cei:condition']"/>
                            <xsl:for-each select="$t">
                                <xsl:call-template name="condition"/>
                            </xsl:for-each>
                        </xsl:if>
                        <xsl:if test="./ead:materialspec[@encodinganalog='cei:nota']">
                            <xsl:variable name="m" select="./ead:materialspec[@encodinganalog='cei:nota']"/>
                    ...
                    <xsl:if test="./ead:materialspec[@encodinganalog='cei:traditioform']">
                        <xsl:variable name="r" select="./ead:materialspec[@encodinganalog='cei:traditioform']"/>
                ...
                <xsl:if test="../ead:odd or ../ead:bibliography">
    ...
    <xsl:template name="oldid">
        <cei:altIdentifier>
            <xsl:value-of select="./ead:unitid[@encodinganalog='cei:altIdentifier']/text()"/>
        </cei:altIdentifier>
    </xsl:template>
    <xsl:template name="unittitle">
        <xsl:if test="./ead:unittitle">
            ...
            <cei:abstract>
                ...
                <xsl:if test="../ead:scopecontent[@encodinganalog='cei:issuer']">
                    <xsl:call-template name="issuer"/>
                </xsl:if>
                <xsl:if test="../ead:scopecontent[@encodinganalog='cei:recipient']">
    ...
    <xsl:template match="ead:unitdate">
        <cei:issued>
            <xsl:choose>
                <xsl:when test="contains(./@normal, '/')">
                    <xsl:element name="cei:dateRange">
                        <xsl:attribute name="from">
                            <xsl:choose>
                                <xsl:when test="string-length(substring-before(./@normal, '/'))&gt;3">
                                    <xsl:value-of select="substring-before(translate(./@normal, '-', ''), '/')"/>
                                </xsl:when>
                        ...
                        <xsl:attribute name="to">
                            <xsl:choose>
                                <xsl:when test="string-length(substring-after(./@normal, '/'))&gt;3">
                                    <xsl:value-of select="substring-after(translate(./@normal, '-', ''), '/')"/>
                                </xsl:when>
        		...
        		<xsl:if test="/ead:materialspec[@encodinganalog='cei:issuedPlace']">
							<cei:place>
								<xsl:value-of select="/ead:materialspec[@encodinganalog='cei:issuedPlace']"/>
							</cei:place>
						</xsl:if>
        </cei:issued>
    </xsl:template>
    ...
    <xsl:template match="ead:physfacet[@encodinganalog='cei:seal']/text()"/>
    <xsl:template match="ead:physfacet[@encodinganalog='cei:sealDesc']/text()"/>
    ...
    <xsl:template name="noterial">
        <cei:notarialSub>
            <xsl:apply-templates/>
        </cei:notarialSub>
    </xsl:template>
    <xsl:template name="issuer">
        <xsl:variable name="x" select="../ead:scopecontent[@encodinganalog='cei:issuer']/ead:p/ead:persname"/>
    ...
    <xsl:template name="recipient">
        <xsl:variable name="x" select="../ead:scopecontent[@encodinganalog='cei:recipient']/ead:p/ead:persname"/>
        <xsl:for-each select="$x">
            <cei:recipient>
                <xsl:value-of select="./text()"/>
            </cei:recipient>
    ...
    <xsl:template name="diplomaticAnalysis">
        <cei:diplomaticAnalysis>
            ...
            <xsl:for-each select="$y">
                <xsl:if test="./@encodinganalog='cei:diplomaticAnalysis' or not(./@encodinganalog='cei:notarialSub')">
                    <cei:p>
            ...
            <xsl:for-each select="$z">
                <xsl:choose>
                    ...
                    <xsl:when test="./@encodinganalog='cei:listBiblEditionsRegesta'">
                    	 <cei:listBiblRegest>
                            <cei:bibl>
                                <xsl:value-of select="./ead:bibref/text()"/>
                            </cei:bibl>
                    ...
                    <xsl:when test="./@encodinganalog='cei:listBiblEditions'">
                        <cei:listBiblEdition>
                            <cei:bibl>
                                <xsl:value-of select="./ead:bibref/text()"/>
                            </cei:bibl>
                    ...
                    <xsl:when test="./@encodinganalog='cei:listBiblRegesta'">
                    	<cei:listBiblRegest>
                            <cei:bibl>
                                <xsl:value-of select="./ead:bibref/text()"/>
                            </cei:bibl>
                    ...
                    <xsl:when test="./@encodinganalog='cei:listBiblStudies'">
                        <cei:listBiblStudies>
                            <cei:bibl>
                                <xsl:value-of select="./ead:bibref/text()"/>
                            </cei:bibl>
                    ...
                    <xsl:when test="./@encodinganalog='cei:listBiblFacsimile'">
                        <cei:listBiblFacsimile>
                            <cei:bibl>
                                <xsl:value-of select="./ead:bibref/text()"/>
                            </cei:bibl>
                        </cei:listBiblFacsimile>
