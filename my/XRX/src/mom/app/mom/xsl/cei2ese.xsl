        ...
        <oai:metadata>
            <europeana:record xmlns="http://www.europeana.eu/schemas/ese/">
                ...
                <xsl:if test=".//cei:issued/cei:date/text() or .//cei:issued/cei:dateRange/text()">
                    <dc:date><!-- <xsl:text>Issued-date: </xsl:text> <xsl:choose> <xsl:when test=".//cei:issued/cei:date/@value"> <xsl:value-of select=".//cei:issued/cei:date/@value"/> </xsl:when> <xsl:otherwise> <xsl:choose> <xsl:when test=".//cei:issued/cei:dateRange/@from = .//cei:issued/cei:dateRange/@to"> <xsl:value-of select=".//cei:issued/cei:dateRange/@from"/> </xsl:when> <xsl:otherwise> <xsl:value-of select=".//cei:issued/cei:dateRange/@from"/> <xsl:text> - </xsl:text> <xsl:value-of select=".//cei:issued/cei:dateRange/@to"/> </xsl:otherwise> </xsl:choose> </xsl:otherwise> </xsl:choose> <xsl:text> (</xsl:text> <xsl:value-of select="concat(.//cei:issued/cei:dateRange/text(), .//cei:issued/cei:date/text())"/> <xsl:if test=".//cei:quoteOriginaldatierung/text()[1]"> <xsl:text>, "</xsl:text> <xsl:value-of select=".//cei:quoteOriginaldatierung/text()[1]/"> </xsl:if> <xsl:text>)</xsl:text> -->
                        <xsl:choose>
                ...
                <xsl:variable name="firstGraphic" select="//cei:graphic[1]/@url" />
                <xsl:if test="$firstGraphic">
                    <europeana:isShownBy>
                        <xsl:variable name="base" select="concat($base-image-url, $firstGraphic)" />
                        <xsl:choose>
                            <xsl:when test="contains($base,'images.monasterium.net')">
                                <xsl:value-of select="concat('http://images.icar-us.eu/iiif/2/', encode-for-uri(substring-after($base, 'images.monasterium.net/')), '/full/512,/0/default.jpg')" />
                            </xsl:when>
                            <xsl:otherwise>
